...
// Tarifs : suppression des anciennes mentions 10 %, suppression de l'ancienne formule et détails cliquables 15 % / 20 %.
document.addEventListener("DOMContentLoaded",()=>{
    ...
    // Supprime complètement toute ancienne carte contenant l'ancien tarif 10 % / Essentiel.
    const allCards=Array.from(document.querySelectorAll(".price-card,.pricing-card,.tarif-card,.offer-card,[class*='price-card'],[class*='pricing-card'],[class*='offer-card']"));
    allCards.forEach(card=>{
        const txt=(card.textContent||"").replace(/\s+/g," ").toLowerCase();
        if(/10\s*%|10\s*pourcent/.test(txt) || /essentiel/.test(txt)) card.remove();
    });

    // Retire également les anciennes lignes/mentions tarifaires qui contiennent encore Essentiel.
    document.querySelectorAll("body *").forEach(el=>{
        if(el.children.length===0){
            const txt=(el.textContent||"").trim().toLowerCase();
            if(txt.includes("essentiel") && /10\s*%|10\s*pourcent/.test(el.textContent||"")) el.remove();
        }
    });
    ...
    function findContactTarget(){
      return document.querySelector("#contact,#contact-form,#contactForm,.contact-section,.contact,footer") || document.body;
    }
    ...
    // Répare les boutons Demander un devis qui n'avaient pas de destination fonctionnelle.
    document.querySelectorAll("a,button").forEach(el=>{
      const txt=(el.textContent||"").replace(/\s+/g," ").trim().toLowerCase();
      if(txt.includes("demander un devis") || txt.includes("nous contacter")){
        if(el.tagName==="A") el.addEventListener("click",goToContact);
        else el.addEventListener("click",goToContact);
      }
    });
    ...
    function ensureModal(){
      ...
      .pricing-detail-btn{margin:18px auto 0;border:1px solid rgba(184,148,77,.45);background:#fff;color:#8f6e32;border-radius:11px;padding:12px 18px;font-family:inherit;font-size:13px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:9px;transition:.25s;box-shadow:0 5px 15px rgba(0,0,0,.08)}
      .pricing-detail-btn:hover{background:#fff8e9;transform:translateY(-2px);box-shadow:0 9px 22px rgba(0,0,0,.12)}
    ...
    const candidates=Array.from(document.querySelectorAll(".price-card,.pricing-card,.tarif-card,.offer-card,[class*='price-card'],[class*='pricing-card'],[class*='offer-card']"));
